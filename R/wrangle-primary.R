...
df2016 <- rio::import("data/anes_timeseries_2016.zip", which = "anes_timeseries_2016_rawdata.txt")%>%
  select(weight = V160101,
         ...
         general_vote = V161026, #Did R vote for president in the general?
         general_vote_choice = V161027, #For whom did R vote for president (NA if they didn't vote)
         ...
         pid_7 = V161158x, # Does R think of themselves as a Dem, Rep, Ind or what?  This is coded 1: strong-dem, 2: weak-dem 3: ind-dem, 4: ind, 5: ind-rep, 6: weak-rep, 7: strong-rep -8 DK, -9 NA
  ...
  mutate(social_classb = recode(social_classb, .default = NA_character_,
                                ...
                                "3" = "Middle Class",
                                "4" = "Upper Class"))%>%
  mutate(social_class = factor(if_else(is.na(social_classa), social_classb, social_classa)),
         levels = c("Lower Class",
                    "Working Class",
                    "Middle Class",
                    "Upper Class"))%>% #2016 ANES used two different question setsfor social class, I'm just uniting them here.
  select(-social_classb,
         -social_classa)%>% #dropping component parts
  mutate(race = recode(race, .default = NA_character_,
                       "1" = "White",
                       "2" = "Black",
  ...
  mutate(primary_vote_choice = as.factor(if_else(is.na(primary_vote_choice) & primary_vote_dum == 0, "Didn't Vote",
                                                 primary_vote_choice)))%>%
